...
float MFM::readVal(uint16_t reg, uint8_t node) {
  ...
  temp = calculateCRC(MFMarr, FRAMESIZE - 3);                                   //calculate out crc only from first 6 bytes
  ...
  if (readErr == MFM_ERR_NO_ERROR) {                                            //if no timeout...
    ...
    if (MFMSer.available() >= FRAMESIZE) {
      ...
      if (MFMarr[0] == node && MFMarr[1] == MFM_B_02 && MFMarr[2] == MFM_REPLY_BYTE_COUNT) {
        ...
        if ((calculateCRC(MFMarr, FRAMESIZE - 2)) == ((MFMarr[8] << 8) | MFMarr[7])) {  //calculate crc from first 7 bytes and compare with received crc (bytes 7 & 8)
          ((uint8_t*)&res)[3]= MFMarr[3];
  ...
  if (readErr != MFM_ERR_NO_ERROR) {                                            //if error then copy temp error value to global val and increment global error counter
...
void MFM::dereSet(bool _state) {
  if (_dere_pin != NOT_A_PIN)
    digitalWrite(_dere_pin, _state);                                            //receive from MFM -> DE Disable, /RE Enable (for control MAX485)
}
